...
beta0=3.566e-7
beta_vec=[ 0., 0.05*beta0, 0.1*beta0, 0.5*beta0, beta0, 1.5*beta0,2*beta0]
...
mpl.rc('axes',  edgecolor='black', linewidth=1, grid=False, titlesize=8.5, labelsize=8.5, labelweight='normal',labelcolor='black')
...
def plot_function_e_and_r (fig, x_vec, y1_vec, y2_vec, y3_vec, xlabel='', y1label=r"$\Delta$e", y2label=r"$\Delta$R/R", lam_key='', e_key=(0.0, 0.0), m_key=''):
    color_fmt=color_dict_e[e_key]
    #plot_pos=plot_positions_six[lam_key]
    label='e=(%g, %g)' %(e_key[0], e_key[1])
    
    print  "x_vec, y1_vec, y2_vec, y3_vec",  x_vec, y1_vec, y2_vec, y3_vec
    ...
    plt.axhline(y=0.,color='k',ls='solid')
    if  e_key[0] == 0.05 and e_key[1] == 0.05:
...
def plot_function_r (fig, x_vec, y3_vec, xlabel='', y2label=r"$\Delta$R/R", lam_key='',m_key='', e_key=0.0, type='g'):
    color_fmt=color_dict_mag [m_key]
    plot_pos=plot_positions_six[lam_key]
    ax = fig.add_subplot (plot_pos)
    label='m=%g'%(m_key)
...
for lam in wavelength_dict:
    for e in e_vec:
        plot_function_e_and_r (fig, beta_vec, np.array(gauss_no_noise[lam][e][0]) , np.array(gauss_no_noise[lam][e][1]), np.array(gauss_no_noise[lam][e][2]), xlabel=r"Nonlinearity parameter $\beta$", lam_key=lam, e_key=e)
plt.suptitle(string_g, fontsize=11)
fig.tight_layout()
